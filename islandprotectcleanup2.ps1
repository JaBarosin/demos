...
try {
   # --- FIX: Ensure \$processes is correctly populated/checked for the running process ---
   $processNameAgent = "Island Service Agent"
   $processes = Get-Process -Name $processNameAgent -ErrorAction SilentlyContinue

   # Check if the process was found
   if ($processes) {
       Write-Host "Found running processes for '$processNameAgent'. Terminating..."
       # Loop through each instance and kill it
       foreach ($process in $processes) {
            Stop-Process -Id $process.Id -Force
            Write-Host "Process ID $($process.Id) terminated."
       }
   } else {
       Write-Host "Process '$processNameAgent' not found."
   }
   # ---------------------------------------------------------------------------------
...
       # --- FIX: Quoted service name for sc.exe delete ---
...
      Write-Host "Registry path '$registryPath' removed."
   } else {
...
      Write-Host "Folder '$islandServicePath' removed."
   }
   if (Test-Path -Path "$islandServiceDataPath") {
       Remove-Item -Path "$islandServiceDataPath" -Force -Recurse -EA SilentlyContinue
       Write-Host "Folder '$islandServiceDataPath' removed."
   }

   # NOTE: Using Get-WmiObject -Class Win32_Product is generally discouraged due to potential side effects.
   # A better practice is to look up the MSI uninstall key directly if the Product Code is known or can be reliably found elsewhere.
   try {
     # Use ErrorAction Stop here to ensure catch block is hit if the WMI call fails
     $product = Get-WmiObject -Class Win32_Product -Filter "Name = '$ProductName'" -ErrorAction Stop
       if (-not $product) {
         Write-Warning "Product $ProductName not found. Exiting registry cleanup."
         return
       }
   }
   catch {
     Write-Warning "No product found with name $ProductName. Exiting registry cleanup."
     return
   }
...
   # --- FIX: Call function and define \$reversedGuid and \$guidNoBraces ---
   $reversedGuid = ConvertTo-RegistryReversedGuid -Guid $productCode
   $guidNoBraces = $productCode -replace '[\{\}]'
